...
async fn test_async_write_then_read_derived_struct() {
    ...
    buf.set_position(0);
    let restored: Header = buf.read_fixed().await.unwrap();
...
async fn test_async_write_read_try_struct() {
    ...
    let mut buf = Cursor::new(Vec::new());
    // Frame implements IntoByteArray → write_fixed
    buf.write_fixed(&original).await.unwrap();

    buf.set_position(0);
    // Frame implements TryFromByteArray → read_fixed
    let restored: Frame = buf.read_fixed().await.unwrap();
    assert_eq!(restored, original);
}
...
async fn test_async_read_try_struct_invalid_discriminant() {
    ...
    let result: std::io::Result<Frame> = buf.read_fixed().await;
...
async fn test_async_write_then_read_try_primitives() {
    ...
    buf.set_position(0);
    let v1: LittleEndian<u16> = buf.read_fixed().await.unwrap();
    let v2: u8 = buf.read_fixed().await.unwrap();
...
async fn test_async_duplex_roundtrip() {
    ...
    let restored: Header = reader.read_fixed().await.unwrap();
    assert_eq!(restored, original);
}
